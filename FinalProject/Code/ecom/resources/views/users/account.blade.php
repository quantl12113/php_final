			...
			<div class="col-sm-4 col-sm-offset-1">
				<div class="login-form"><!--login form-->
					...
					<form id="accountForm" name="accountForm" action="{{ url('/account') }}" method="POST">
						{{ csrf_field() }}
						<input id="name" name="name" type="text" placeholder="Name" value="{{ $userDetails->name }}" />
						...
						<select id="country" name="country">
							...
							@foreach($countries as $country) 
								<option value="{{ $country->country_name }}" <?php if($country->country_name == $userDetails->country) echo "selected" ?>>{{ $country->country_name }}</option>
							@endforeach
			...
			<div class="col-sm-4">
				<div class="signup-form"><!--sign up form-->
					...
					<form id="passwordForm" name="passwordForm" action="{{ url('/update-user-pwd') }}" method="POST">
						...
						<span id="chkPwd"></span>
						<input id="new_pwd" type="password" name="new_pwd" id="new_pwd" placeholder="New Password">
						<input id="confirm_pwd" type="password" name="confirm_pwd" id="confirm_pwd" placeholder="Confirm Password">
						<span id="chk_confirm"></span>
						<button type="submit" class="btn btn-default">Update</button>
					</form>
				</div><!--/sign up form-->
			</div>
