...
class ProductsController extends Controller
{
    ...
    public function checkout(Request $request) {
        ...
        if($request->isMethod('post')) {
            ...
            User::where('id', $user_id)->update(['name'=>$data['billing_name'], 'address'=>$data['billing_address'], 'city'=>$data['billing_city'], 'state'=>$data['billing_state'], 'pincode'=>$data['billing_pincode'], 'mobile'=>$data['billing_mobile'], 'country'=>$data['billing_country']]);

             if($shippingCount > 0) {
                DeliveryAddress::where('user_id', $user_id)->update(['name'=>$data['shipping_name'], 'address'=>$data['shipping_address'], 'city'=>$data['shipping_city'], 'state'=>$data['shipping_state'], 'pincode'=>$data['shipping_pincode'], 'mobile'=>$data['shipping_mobile'], 'country'=>$data['shipping_country']]);
            }else {
